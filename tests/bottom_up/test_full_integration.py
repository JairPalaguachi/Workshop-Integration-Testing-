...
def test_pedido_rechazado_no_cambia_estado(requests_mock, integrated_service):
    service, db = integrated_service
    requests_mock.post(PAY_URL, status_code=402, json={"reason": "insufficient_founds"})
...
def test_dos_pedidos_descuentan_stock_acumulado(requests_mock, integrated_service):
    service, db = integrated_service
    requests_mock.post(PAY_URL, status_code=200, json={"transaction_id": "tx_1"})
    resultado1 = service.place_order("SKU-1", 2, "tok_visa")
    resultado2 = service.place_order("SKU-1", 3, "tok_visa")
    assert resultado1["total"] == 232.0
    assert resultado1["status"] == "CONFIRMED"
    assert resultado2["total"] == 348.0
    assert resultado2["status"] == "CONFIRMED"
    assert db.get_stock("SKU-1") == 5
    assert resultado1["order_id"] in db.orders
    assert resultado2["order_id"] in db.orders
